...
function setHostPlayerLists(active, inactive) {
  ...
  const mkRow = (name, online) => {
    const n = escapeHtml(name);
    const cmdName = escapeHtml(name);
    const actions = online
      ? `
        <button type="button" class="btn btn-ghost btn-sm host-pl-btn" data-pl-act="heal" data-pl="${cmdName}">Heal</button>
        <button type="button" class="btn btn-ghost btn-sm host-pl-btn" data-pl-act="kick" data-pl="${cmdName}">Kick</button>
        <button type="button" class="btn btn-danger btn-sm host-pl-btn" data-pl-act="ban" data-pl="${cmdName}">Ban</button>
      `
      : `
        <button type="button" class="btn btn-danger btn-sm host-pl-btn" data-pl-act="ban" data-pl="${cmdName}">Ban</button>
      `;
    return `
      <li class="host-player-row">
        <span class="host-player-name">${n}</span>
        <span class="host-player-actions">${actions}</span>
      </li>
    `;
  };
  if (aEl) {
  ...
  // Re-render with actions when possible
  if (aEl && active && active.length) aEl.innerHTML = active.map((n) => mkRow(n, true)).join('');
  if (iEl && inactive && inactive.length) iEl.innerHTML = inactive.map((n) => mkRow(n, false)).join('');
}
...
async function syncHostConsoleFromMain(serverId) {
  ...
  try {
    ...
    const cfg = (servers || []).find((x) => x.id === serverId) || null;
    const ram = document.getElementById('host-server-ram-mb');
    if (ram && cfg?.maxRamMb) ram.value = String(cfg.maxRamMb);
  } catch (e) {}
}
...
function bindHostConsolePanelControls() {
  ...
  document.getElementById('host-server-ram-save')?.addEventListener('click', async () => {
    if (!hostConsoleServerId || !window.zenon.serverUpdate) return;
    const el = document.getElementById('host-server-ram-mb');
    const n = parseInt(el?.value || '2048', 10);
    const mb = Number.isFinite(n) ? Math.max(512, Math.min(16384, n)) : 2048;
    const res = await window.zenon.serverUpdate(hostConsoleServerId, { maxRamMb: mb });
    if (res?.success) showToast('Server RAM saved', 'success', 1600);
    else showToast(res?.error || 'Could not save RAM', 'error', 3500);
  });
  ...
  // Player actions (kick/ban/heal)
  document.querySelector('.host-console-sidebar')?.addEventListener('click', async (e) => {
    const btn = e.target.closest('[data-pl-act]');
    if (!btn || !hostConsoleServerId) return;
    const act = btn.getAttribute('data-pl-act');
    const name = btn.getAttribute('data-pl') || '';
    if (!act || !name) return;
    if (act === 'kick') {
      await window.zenon.serverSendCommand(hostConsoleServerId, `kick ${name}`);
      showToast(`Kicked ${name}`, 'info', 1600);
    } else if (act === 'ban') {
      const ok = confirm(`Ban ${name}?`);
      if (!ok) return;
      await window.zenon.serverSendCommand(hostConsoleServerId, `ban ${name}`);
      showToast(`Banned ${name}`, 'info', 1600);
    } else if (act === 'heal') {
      // Modern + legacy fallback
      await window.zenon.serverSendCommand(hostConsoleServerId, `effect give ${name} minecraft:regeneration 5 255 true`);
      await window.zenon.serverSendCommand(hostConsoleServerId, `effect ${name} regeneration 5 255 true`);
      showToast(`Healed ${name}`, 'success', 1400);
    }
  });
...
async function renderHostServer() {
    ...
    <div class="page page-host-server">
      ...
      <div class="host-server-create glow-card">
        ...
        <div class="form-group">
          ...
          <input type="number" id="host-server-maxram" class="form-input" min="512" max="16384" step="128" value="2048" />
          <p class="auth-hint">Applies when you start the server. You can change it later in Console &amp; controls.</p>
        </div>
        <div class="form-group">
        ...
        <div class="host-console-grid">
          ...
          <aside class="host-console-sidebar">
            ...
            <div class="card" style="padding:12px;margin-top:12px">
              <div style="font-weight:700;font-size:12px;margin-bottom:8px">Server RAM</div>
              <div style="display:flex;gap:10px;align-items:center">
                <input type="number" class="form-input" id="host-server-ram-mb" min="512" max="16384" step="128" value="2048" style="width:120px" />
                <button type="button" class="btn btn-ghost btn-sm" id="host-server-ram-save">Save</button>
              </div>
              <div style="margin-top:6px;color:var(--text-muted);font-size:11px">MB (applies next start)</div>
            </div>
  ...
  document.getElementById('host-create-server')?.addEventListener('click', async () => {
    ...
    const maxRamMb = parseInt(document.getElementById('host-server-maxram')?.value || '2048', 10) || 2048;
    if (!version) {
