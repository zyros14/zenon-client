...
class LiveTokenManager {
  ...
  async refreshTokens () {
    ...
    const codeRequest = {
      ...
      headers: {
        ...
        'User-Agent': 'MinecraftLauncher/2.2.10675 (Electron)'
      },
  ...
  async authDeviceCode (deviceCodeCallback) {
    ...
    let pollIntervalSec = Math.max(1, Number(res.interval) || 5)
          ...
          .then(res => res.json()).then(res => {
            if (res.error) {
              if (res.error === 'authorization_pending') {
                ...
                // RFC 8628: increase polling interval; otherwise prismarine throws and spins until expiry.
                pollIntervalSec = Math.min(60, pollIntervalSec + 5)
                debug('[live] slow_down, backing off to', pollIntervalSec, 's')
              } else {
